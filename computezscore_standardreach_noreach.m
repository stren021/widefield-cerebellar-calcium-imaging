%%%%computes z score maps aligned to auditory tone when the mouse failed to
%%%%reach

dir_all = full_data_directory_here;

file_name_out = [dir_all, 'zscores_beep_longwin_noreach_IC'];
...
for s_ix = 1:stack_num
    ...
    btimes = find(event_mat == 1);
    dtimes = find(event_mat == 2);
    ttimes = find(event_mat == 3);
    rtimes = find(event_mat == -1);
    
    rtimes_2D = reshape(rtimes, [length(rest), stack_num])';
    ...
    for dendsom = 1:2
        ...
        %DFF_MAP_BEEP = NaN(im_size^2,(length(tau_beep)-1));
             ...
             for event_type = 1
                 switch event_type
                     case 1
                         ...
                         bwin = 0:50:3000;
                         taus = tau_beep;
                         dff_map = DFF_MAP_BEEP;

                 end
             

                        collect_ind_IC = NaN(1, (length(tau_beep)-1));
                        ...
                        dfwin_collect = [];
                         ...
                         for tau_ix = 1:(length(taus)-1)
                             ...
                             if abs(dfmean_bin) >= 2
                             collect_ind_IC(tau_ix)  = dfmean_bin;
                             end
