...
mouse_dir = full_mouse_data_directory_here %'G:\MSI\PcP2-base-050\';
day_list = dir(mouse_dir)
day_list = day_list(~ismember({day_list.name},{'.','..'}));

for day_ix = 1:length(day_list) %1:length(day_list)%2:length(day_list)
    
    day_dir = [day_list(day_ix).folder, '\' day_list(day_ix).name, '\', 'stacks', '\']

    zoom_list = dir(day_dir);
    zoom_list = zoom_list(~ismember({zoom_list.name},{'.','..'}));
    
    for zoom_ix = 1:length(zoom_list)
        
        stack_dir = [zoom_list(zoom_ix).folder, '\' zoom_list(zoom_ix).name, '\'];
        folder_dir=dir([stack_dir, '0*']);
        
        file_name_out = [stack_dir 'compressed_data']; 

%         this is where you need to initialize the hdata concatenation
%         stack
            ...
            ind_end = ind_start + (corrected_length-1);
                ...
                for f=1:length(files_list)
                    ...
                    if length(fname)==lmin;
                        ...
                        fseq=str2num(fname(end-6:end-4));
                        file_order(fseq)=f;
                    end
                end


                data=nan(yDim,xDim,stack_length);
