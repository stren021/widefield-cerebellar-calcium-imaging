...
%sliding window correlation analysis for reach vs catch in the catch trial
%paradigm

dir_all = full_data_directory_here

file_name_out = [dir_all, 'corrmovav_catchreach_DEND_2sec'];
...
collect_av_within_catch = [];
collect_av_between_catch = [];
collect_av_within_reach = [];
collect_av_between_reach = [];

collect_z_within_catch = [];
collect_z_between_catch = [];
collect_z_within_reach = [];
collect_z_between_reach = [];

COLLECT_ALL_WITHIN_CATCH = []
COLLECT_ALL_BETWEEN_CATCH = [];
...
collect_within_catch = [];
collect_between_catch = [];
...
%     tcurr = tcurr(p_true);
    ...
    for beep_ix = 1:length(pcurr)
        

            

                    
                    b_window = pcurr(beep_ix)-pre_beep_win:pcurr(beep_ix)+post_beep_win;
                    stack_correct = (s_ix-1)*stack_length;
                    b_window = b_window + stack_correct;
                    
                    beep_window_catch = [beep_window_catch; b_window];
       ...
       for beep_ix = 1:length(bcurr)
        ...
        if beep_ix < length(bcurr)
           ...
           for IC_ix = 1:length(IC_list);
                ...
                %get max size domain for between correlations
                length_mat = [];
                for d_ix = 1:length(Reg_list)
                    d_length = find(IC_map_2D == d_ix);
                    length_mat = [length_mat; length(d_length)];
                end
                
                d_max = find(length_mat == max(length_mat));
                maxdom_mat = [maxdom_mat; [IC d_max]];
                
                IC_curr = IC_all(d_max,:);
           ...
           %%%%%rest correlation calculations%%%%%
                ...
                for pix = 1:2
                    switch pix
                        ...
                        case 2
                            time_mat = beep_window_catch;
                            taus = reach_taus;
                            
                            collect_all_between = collect_between_catch;
                    end
                     collect_within = [];
                     collect_between = [];
                ...
                switch pix 
                    ...
                    case 2
                        ...
                        collect_between_catch = collect_all_between;
                end
                end
 ...
 between_av_catch = mean(collect_between_catch);
 ...
 collect_av_between_catch = [collect_av_between_catch [mouse_ix; day_ix; between_av_catch']];
 ...
 between_z_catch = (collect_between_catch./collect_av_between_rest);

 between_z_catch_mean = mean(collect_between_catch./collect_av_between_rest);
 


 figure(mouse_ix*10+day_ix);subplot(4,2,3);plot(plot_taus, between_av_reach);
 figure(mouse_ix*10+day_ix);subplot(4,2,4);plot(plot_taus, between_z_reach_mean);


 figure(mouse_ix*10+day_ix);subplot(4,2,7);plot(plot_taus, between_av_catch);
 figure(mouse_ix*10+day_ix);subplot(4,2,8);plot(plot_taus, between_z_catch_mean);
...
collect_z_between_catch = [collect_z_between_catch [mouse_ix; day_ix; between_z_catch_mean']];
 ...
 COLLECT_ALL_BETWEEN_CATCH = [COLLECT_ALL_BETWEEN_CATCH; between_z_catch];
...
figure(1212121);hold on;subplot(4,1,2);imagesc(COLLECT_ALL_BETWEEN_REACH); caxis([0 1.5])

figure(1212121);hold on;subplot(4,1,4);imagesc(COLLECT_ALL_BETWEEN_CATCH); caxis([0 1.5])

save(file_name_out,  'COLLECT_ALL_BETWEEN_REACH',...
    'collect_z_between_reach', 'collect_av_between_reach',...
 'COLLECT_ALL_BETWEEN_CATCH', ...
    'collect_z_between_catch', 'collect_av_between_catch');
