%%%%computes z score maps aligned to auditory tone when the mouse made a
%%%%reach

dir_all = full_data_directory_here;

file_name_out = [dir_all, 'zscores_beep_longwin_truereach'];
...
for s_ix = 1:stack_num
    ...
    event_mat_ix(rest) = -1;
        ...
        if ~isempty(touch_ind)
            ...
            if (length(reach_win)/fs) <= 3

            event_mat_ix(touch_ind(1)) = 3;
            event_mat_ix(bcurr(beep_ix)) = 1;
            event_mat_ix(dcurr(beep_ix)) = 2;
            end
        end
        end
